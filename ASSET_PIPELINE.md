...
## M9 Procedural Placeholder Batch

M9 may use deterministic local procedural images when no approved external AI generation tool is available.

Rules:
- Prompt files must still exist for every AI-intended replacement group.
- Procedural files must be generated by a repeatable script.
- Manifest entries must label these assets as procedural placeholders, not final AI-generated art.
- Babylon materials and decorative planes must load from stable public asset paths.
- Validation must confirm prompt refs, source files, manifest entries, and smoke proof.
